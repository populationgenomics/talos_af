...
"""
Takes a MatrixTable, a BED file, and an output path
Region-filters the MT, and writes it out as a single VCF
"""
...
def main(
    ...
    Args:
        mt_path (str):
        output (str): write region-filtered MatrixTable, stripped of INFO fields
    ...
    bed_region = hl.import_bed(bed, reference_genome=config.genome_build(), skip_invalid_intervals=True)
    ...
    loguru.logger.info('Writing VCF as a single fragment.')
    hl.export_vcf(
        mt,
        output,
        tabix=True,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', help='Path to the input MT', required=True)
    parser.add_argument('--output', help='Path to write the resulting VCF', required=True)
